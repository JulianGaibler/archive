...
export function parseArchiveTT(text: string): CaptionTrack | null {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      // Parse placement
      const placementMatch = rest.match(/^\[(top|left|right)\]\s*/)
      if (placementMatch) {
        currentCue.placement = placementMatch[1] as Placement
        rest = rest.slice(placementMatch[0].length)
      }

      // Parse voice
      const voiceMatch = rest.match(/^<([^>]+)>\s*/)
      if (voiceMatch) {
        currentCue.voice = voiceMatch[1]
        rest = rest.slice(voiceMatch[0].length)
      }
