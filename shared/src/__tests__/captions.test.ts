...
describe("parseArchiveTT", () => {
  ...
  it("parses placement via ^^ delimiter", () => {
    const result = parseArchiveTT("@0:05^^top Hello");
    expect(result!.cues[0].placement).toBe("top");
    expect(result!.cues[0].voice).toBeUndefined();
    expect(result!.cues[0].text).toBe("Hello");
  });

  it("parses voice and placement together", () => {
    const result = parseArchiveTT("@0:05^Bob^left Hello");
    expect(result!.cues[0].placement).toBe("left");
    expect(result!.cues[0].voice).toBe("Bob");
    expect(result!.cues[0].text).toBe("Hello");
  });

  it("treats brackets in text as plain text", () => {
    const result = parseArchiveTT("@0:05 [Applause]");
    expect(result!.cues[0].text).toBe("[Applause]");
    expect(result!.cues[0].placement).toBeUndefined();
  });
