 ...
 *
 * This function uses the current working directory as the base path
 * and appends the provided path segments to it. It ensures that the
 * resulting path is normalized and absolute.
 *
 ...
 * recursive deletion for directories and retries up to 3 times in case of failure.
 *
 ...
 *
 * This function uses `fs.rmSync` to delete the target path recursively
 * and forcefully, with a maximum of 3 retries in case of failure.
 *
 * @param targetPath - The path to the file or directory to be removed.
...
/**
 * Ensures that a directory exists at the specified path, optionally creating it,
 * emptying it, or setting its permissions.
 *
 * @param targetPath - The path of the directory to ensure.
 * @param criteria - Optional criteria for directory handling.
 * @param criteria.empty - If `true`, ensures the directory is empty by removing all its contents.
 * @param criteria.mode - The file mode (permissions) to set for the directory. Can be a number or an octal string.
 *
 * @throws Will throw an error if the path exists but is not a directory, or if any file operation fails.
 */
export async function dirAsync(targetPath: string, criteria?: { empty?: boolean; mode?: number | string }): Promise<void> {
...
/**
 * Ensures that a directory exists at the specified path, optionally clearing its contents
 * and setting its permissions mode.
 *
 * @param targetPath - The path of the directory to ensure.
 * @param criteria - Optional criteria for the directory.
 * @param criteria.empty - If `true`, ensures the directory is empty by removing all its contents.
 * @param criteria.mode - The permissions mode to set for the directory. Can be a numeric value or an octal string.
 *
 * @throws Will throw an error if the path exists but is not a directory, or if any filesystem operation fails.
 */
export function dir(targetPath: string, criteria?: { empty?: boolean; mode?: number | string }): void {
  const { empty = false, mode } = criteria || {};
 ...
 * @param options.overwrite - If `true`, overwrites the destination if it already exists. Default is `false`.
 *
 * @throws Will throw an error if the move operation fails.
 */
export async function moveAsync(from: string, to: string, options?: { overwrite?: boolean }): Promise<void> {
 ...
 * @throws Will throw an error if the move operation fails.
 */
export function move(from: string, to: string, options?: { overwrite?: boolean }): void {
  const { overwrite = false } = options || {};
