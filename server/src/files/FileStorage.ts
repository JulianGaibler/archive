...
export default class FileStorage {
    ...
    async storeFile(ctx: Context, upload: Promise<FileUpload>, serializedItem: string) {
        ...
        const taskId = await TaskActions.mCreate(ctx, { ext, serializedItem, mimeType: mime })

        try {
            await this.streamToFile(typedStream, [options.dist, options.directories.queue, taskId.toString()])
        } catch (error) {
    ...
    private async checkQueue() {
        ...
        try {
            if (await TaskActions.qCheckIfBusy(ctx)) { return }
    ...
    private async processItem(ctx: Context, taskId: number, itemData: any) {
        const filePath = jet.path(options.dist, options.directories.queue, taskId.toString())
    ...
    private async performCleanup() {
        ...
        try {
            ...
            ids.forEach(id => jet.remove(jet.path(options.dist, options.directories.queue, id.toString())))
        } finally {
            release()
            this.checkQueue()
        }
