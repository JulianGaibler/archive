...
export default class KeywordModel extends UniqueModel {
    ...
    static getLoaders() {
        ...
        const getByPost = new DataLoader<number, KeywordModel[]>(this.keywordsByPost)

        return { getById, getByPost }
    }

    private static async keywordsByIds(keywordIds: number[]): Promise<KeywordModel[]> {
        const keyword = await KeywordModel.query().findByIds(keywordIds)
    ...
    private static async keywordsByPost(postIds: number[]): Promise<KeywordModel[][]> {
        const posts: any = await PostModel.query()
            .findByIds(postIds)
            .select('post.id', 'keywords')
            .eagerAlgorithm(PostModel.JoinEagerAlgorithm)
            .eager('keywords')
