...
const createPost: GraphQLFieldConfig<any, any, any> = {
    ...
    resolve: (parent, args, ctx: Context) => {
        ...
        if (args.keywords && args.keywords.length > 0) {
            keywords = args.keywords.map(globalId => HashId.decode(keywordHashType, globalId))
        }
...
const editPost: GraphQLFieldConfig<any, any, any> = {
    ...
    resolve: async (parent, args, ctx: Context) => {
        let keywords
        if (args.keywords && args.keywords.length > 0) {
            keywords = args.keywords.map(hashId => HashId.decode(keywordHashType, hashId))
        }
...
const deletePosts: GraphQLFieldConfig<any, any, any> = {
    ...
    resolve: async (parent, args, ctx: Context) => {
        const postIds = args.ids.map(hashId => HashId.decode(postHashType, hashId))
