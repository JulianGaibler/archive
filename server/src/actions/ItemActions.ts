...
export default class {
    ...
    static async qItems(ctx: Context, fields: { limit?: number, offset?: number, byUsers?: number[], byKeywords?: number[], byTypes?: string[], byLanguage?: string, byContent?: string }) {
        ctx.isAuthenticated()
        ...
        if (fields.byContent && fields.byContent.trim().length > 0) {
            const tsQuery = fields.byContent.split(' ').map(k => (`${k.replace(/[;/\\]/g, '')}:*`)).join(' & ')
            query
                .joinRaw('INNER JOIN ( SELECT id, SEARCH FROM item_search_view WHERE SEARCH @@ to_tsquery(?)) b ON b.id = item.id', tsQuery)
        ...
        const [data, totalSearchCount, totalCount] = await Promise.all([
            query
                ...
                .then(rows => {
                    rows.forEach(x =>
                        ctx.dataLoaders.item.getById.prime(x.id, x),
                    )
                    return rows
                }),
            query
    ...
    /// Mutations
    static async mUpload(ctx: Context, fields: { postId: number, file: Promise<FileUpload>, caption?: string, description?: string, type?: string }) {
        ctx.isAuthenticated()
        ...
        const serializedItem = JSON.stringify(localModel.toJSON({ shallow: true, virtuals: false }))

        const taskId = await Context.fileStorage.storeFile(ctx, fields.file, serializedItem)

        return TaskActions.qTask(ctx, { taskId })
    }

    static async mCreate(ctx: Context, fields: { postId: number, caption?: string, description?: string, type?: string, relHeight: number, compressedPath: string, thumbnailPath: string, originalPath: string }) {
        ctx.isServerContext()
    ...
    static async mUpdate(ctx: Context, fields: { itemId: number, changes: any }) {
        ctx.isAuthenticated()
        return ItemModel.query().findById(fields.itemId).patchAndFetch(fields.changes)
    }

    static async mDelete(ctx: Context, fields: { itemIds: number[] }) {
        ctx.isAuthenticated()

        const items = await ItemModel.query().findByIds(fields.itemIds).withGraphFetched('post')
        items.forEach((item: ItemModel) => {
            if (item.post.id !== ctx.userIId) {
