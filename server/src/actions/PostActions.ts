...
export default class {
    ...
    static async qPosts(ctx: Context, fields: { limit?: number, offset?: number, byUsers?: number[], byKeywords?: number[], byTypes?: string[], byLanguage?: string, byContent?: string }) {
        ctx.isAuthenticated()
    ...
    /// Mutations
    static async mCreate(ctx: Context, fields: { title: string, language: string, keywords?: number[] }) {
        ctx.isAuthenticated()
        const postData = {
            title: fields.title,
            language: fields.language,
            creatorId: ctx.userIId,
    ...
    static async mEdit(ctx: Context, fields: { postId: number, title?: string, language?: string, keywords?: number[] }) {
        ctx.isAuthenticated()
        const postData = {
            id: fields.postId,
            title: fields.title || undefined,
            language: fields.language || undefined,
            keywords: fields.keywords ? fields.keywords.map(id => ({ id })) : undefined,
        }
        const [updatedPost] = await PostModel.query().upsertGraphAndFetch([postData], {
            relate: true,
        })
        // TODO: Error handling
        return updatedPost
