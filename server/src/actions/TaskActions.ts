...
export default class {
    ...
    static async qTasks(ctx: Context, fields: { limit?: number, offset?: number, byUsers?: number[], byStatus?: string[] }) {
        ctx.isAuthenticated()
    ...
    static async qCheckIfBusy(ctx: Context) {
        ctx.isServerContext()
        const activeTasks = await TaskModel
            .query()
    ...
    /// Mutations
    static async mCreate(ctx: Context, fields: { ext: string, serializedItem: string, mimeType: string }) {
        ctx.isAuthenticated()
        const task = await TaskModel.query().insert({
            uploaderId: ctx.userIId,
            ext: fields.ext,
    ...
    static async mUpdate(ctx: Context, fields: { taskId: number, changes: any }) {
        ...
        if (fields.changes.notes) {
            await TaskModel.query().findById(fields.taskId).patch({
                notes: raw('CONCAT(notes, ?::text)', fields.changes.notes),
            })
            delete fields.changes.notes
        }

        const updatedTask = await TaskModel.query().findById(fields.taskId).patchAndFetch(fields.changes)
        Context.pubSub.publish('taskUpdates', {
            id: updatedTask.id,
            kind: 'CHANGED',
            task: updatedTask,
    ...
    static async mPopQueue(ctx: Context) {
        ...
        await this.mUpdate(ctx, { taskId: task.id, changes: { status: 'PROCESSING' } })
